...
function AddHomeModal({
  ...
  const [form, setForm] = useState<AddHomeForm>({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-md overflow-hidden">
        ...
        <div className="px-6 py-5 space-y-4 max-h-[60vh] overflow-y-auto">
          {(["name", "address", "phone", "email", "responsible_individual"] as const).map((key) => (
            <div key={key}>
              <label className="block text-xs font-semibold text-slate-500 mb-1.5 uppercase tracking-wider">
                {key === "responsible_individual" ? "Responsible Individual" : key.charAt(0).toUpperCase() + key.slice(1)}
              </label>
              <input type={key === "email" ? "email" : key === "phone" ? "tel" : "text"} value={form[key]}
...
export default function OrganisationsClient({ dbOrgs, dbHomes, dbStaff }: OrganisationsClientProps) {
  ...
  function getOrgBillingRow(orgId: string): OrgBillingRow | null {
    ...
    if (!org) return null;
    const homeCount = localHomes.filter(h => h.organisation_id === orgId).length;
    return {
  ...
  const [localHomes, setLocalHomes] = useState<DbHome[]>(dbHomes);
  ...
  async function toggleHomeStatus(home: DbHome) {
    const newStatus = home.status === "active" ? "inactive" : "active";
    setLocalHomes(prev => prev.map(h => h.id === home.id ? { ...h, status: newStatus } : h));
    await updateHome(home.id, { status: newStatus });
  }

  async function addHomeForOrg(form: AddHomeForm): Promise<{ error?: string }> {
  ...
  return (
    <>
      ...
      {addingHomeForOrg && (
        <AddHomeModal
          organisationId={addingHomeForOrg.id}
          onClose={() => setAddingHomeForOrg(null)}
          onAdd={addHomeForOrg}
        />
      )}
      {editingHome && (
                    ...
                    {isExpanded && (
                      <div className="bg-slate-50 border-t border-slate-100 px-6 py-3">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <button
                            onClick={() => setAddingHomeForOrg(org)}
